...
To execute the program we use the following syntax:
  ...
  1)  $ python maxconnect4.py one-move input.txt output.txt depth

  2)  $ python maxconnect4.py interactive input.txt [humam-next/computer-next] depth

For example :  1) python maxconnect4.py one-move input.txt output.txt 3
                
               2) python maxconnect4.py interactive input.txt computer-next 6
Note:
-> It is important to give the initial state for the program, that is input.txt
-> The program plans to win the game at the last stage and not intermediate levels
